...
| **Region**            | AWS region (e.g., `eu-west-1`)              | No       |
...
:::tip Least privilege
Anaphora only uploads files (`s3:PutObject`). For write-only archiving, you can remove `s3:GetObject` and
`s3:ListBucket`.
:::
...
2. Click **Test**
3. In **Test pdf**, click **Select pdf** and select a PDF file
4. Optionally, enter a **Test Key** (the object name, without `.pdf`)
5. Click **Upload to S3**
6. Make sure the file appears in your bucket
